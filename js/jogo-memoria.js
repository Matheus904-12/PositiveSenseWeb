/**
 * ========================================
 * POSITIVESENSE - JOGO DA MEMÓRIA
 * ========================================
 *
 * Jogo interativo de memória com 16 cartas (8 pares)
 *
 * Recursos:
...
class MemoryGame {
    ...
    alertDiv.innerHTML = `
            <div class="custom-alert-content">
                ...
                <button class="btn-primary" onclick="this.parentElement.parentElement.remove(); ${
                  callback ? "window.currentCallback()" : ""
                }">
                    OK
                </button>
            </div>
    ...
    cardImages.forEach((image, index) => {
      ...
      card.innerHTML = `
                <div class="card-face card-back">
                    <img src="img/teladecarregamento.gif" alt="PositiveSense" style="width: 40%; height: 40%; object-fit: contain;">
                </div>
  ...
  updateStats() {
    ...
    document.getElementById(
      "pairs"
    ).textContent = `${this.matchedPairs}/${this.images.length}`;
  }
  ...
  saveBestTime() {
    ...
    if (!best || this.timer < parseInt(best)) {
      ...
      this.showCustomAlert(
        "🏆 NOVO RECORDE!",
        `Você estabeleceu um novo melhor tempo: ${
          document.getElementById("timer").textContent
        }! 🎉`,
        null
      );
